...
export default function ReportPage() {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // Create a clone with proper PDF styling
      const reportClone = reportRef.current.cloneNode(true) as HTMLElement;
      reportClone.style.width = '210mm';
      reportClone.style.padding = '20mm';
      reportClone.style.fontSize = '12pt';
      reportClone.style.background = '#FCF2DF';
      document.body.appendChild(reportClone);
  
      // Remove elements that shouldn't be in PDF
      const elementsToRemove = reportClone.querySelectorAll('.download-btn, .back, .navigation');
      elementsToRemove.forEach(el => el.remove());
  
      // Generate canvas with all content
      ...
      // Add content in chunks that fit on A4 pages
      let heightLeft = imgHeight;
      let position = margin;
      const pageContentHeight = pageHeight - (margin * 2);
  
      // Add first page with header
      pdf.setFillColor(254, 220, 182);
      pdf.rect(0, 0, pageWidth, 15, 'F');
      pdf.setFontSize(16);
      pdf.setTextColor(60, 51, 52);
      pdf.text('Seasonal Color Report', pageWidth / 2, 10, { align: 'center' });
      ...
      pdf.text(`Generated by ColoriAI • ${new Date().toLocaleDateString()}`, 
